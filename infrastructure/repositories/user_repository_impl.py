...
class UserRepository(IUserRepository):
    ...
    def find_by_id(self, user_id: int) -> User:
        ...
        if db_user:
            return User(user_id=db_user.id, name=db_user.name, created_date=db_user.created_date)
        return None
    ...
    def list_users(self) -> List[User]:
        ...
        return [User(user_id=user.id, name=user.name, created_date=user.created_date) for user in db_users]
